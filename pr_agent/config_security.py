...
REPO_HOST_ONLY_KEYS_BY_SECTION = {
    ...
    # description_issue_regex is compiled with re.compile and run with finditer over the whole
    # pull-request description, which is attacker-supplied and not length-capped. A pattern whose
    # matching is ambiguous backtracks exponentially, so a caller who can choose it can burn a
    # worker for an unbounded time on a short body. The setting is only meaningful as an operator
    # choice, and this is what makes it one: without it, [config] is otherwise repo-configurable,
    # so a reviewed repo's .pr_agent.toml or a /review --config.description_issue_regex=... comment
    # argument could supply the pattern. The operator still sets it through host configuration.
    "config": frozenset({
